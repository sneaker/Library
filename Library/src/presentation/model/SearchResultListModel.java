...
public class SearchResultListModel extends Observable implements ListModel {
	...
	private void newsearch() {
		 ...
		 for (Searchable item : displayed_results) {
			 if (item.searchTitle().toLowerCase().contains(searchstring))
				 ...
				 for (Attribute att : item.searchDetail().asList()) {
						...
						if (att.getValue().toString().toLowerCase().contains(searchstring)) {
							tmplist.add(item);
							break;
						}
				}
		 ...
		 setChanged();
		 notifyObservers();
		 System.out.println("newsearch finished with: " + searchstring + " and found " + displayed_results.size() + " entries");
	}
	 ...
	 public void delchar() {
		 if (searchstring.length() > 0) {
			 ...
			 System.out.println("delchar called, new string is: " + searchstring);
		 }
		 setChanged();
		 notifyObservers();
